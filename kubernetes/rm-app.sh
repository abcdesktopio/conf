...
opts=$(getopt \
    --longoptions "help,version,clean,force:,imageid:,namespace:," \
    --name "$(basename "$0")" \
    --options "" \
    -- "$@"
)
...
if [ -z ${IMAGEID+x} ]; then 
	echo "--imageid is unset";
	display_message "Usage: $(basename $0) --imageid  <sha_id>] [--namespace <namespace>]" "ERROR";
	exit 1
fi
...
# define service URL 
# inside pyos
URL="http://localhost:8080/API/manager/image/$IMAGEID"
kubectl exec -n "$NAMESPACE" -t "$PYOS_POD_NAME" -- curl -X DELETE "$URL" 
